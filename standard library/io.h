...
;---------------------------------------------------------------------------
ftoa		MACRO	source, result
			finit
			push	source
			push	result           ; push parameter
			call	ftoaproc       ; atof(source)
			ENDM
;---------------------------------------------------------------------------
atof		MACRO	source, result
			finit
			push	source           ; push parameter
...
;   fraction: doubleword (with leading 1)
	...
	while1:   
				mov		eax, fraction   ; copy fraction
